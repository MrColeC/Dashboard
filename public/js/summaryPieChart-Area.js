...
var pieChart_area = function(){
  // Zero out current content
  $("#pieSummaryArea").empty();
  ...
  var svg;
  if (width >= 500) {
    svg = d3.select('#pieSummaryArea').append('svg').attr('width', width).attr('height', height).append('g').attr('transform', 'translate(' + (width / 4) +',' + (height / 2) + ')');
  } else {
    svg = d3.select('#pieSummaryArea').append('svg').attr('width', width).attr('height', height).append('g').attr('transform', 'translate(' + (width / 2) +',' + (height / 2) + ')');
  }
  ...
  // Enable tooltips
  var tooltip = d3.select('#pieSummaryArea').append('div').attr('class', 'd3tooltip');
